...
pub fn sub_domain_iterator(domain: &str, min: usize) -> impl Iterator<Item = &str> {
  domain.char_indices().rev().filter(|(_i, c)| *c == '.').skip(min).map(move |(i, _c)| &domain[i + 1..])
}
...
fn domain_constructor_test() {
  let nd = vec!["# just a comment", "   # just a comment", "localhost"];
  ...
  let v = vec!["domain.com", "domain.com # domain and comment", "10.0.0.1 domain.com", " 10.0.0.1  domain.com # comment"];
